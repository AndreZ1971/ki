...
export const trendAggregatorRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    const CATEGORY_SEEDS: Record<string, string[]> = {
      electronics: [
        'smart home', 'wireless charger', 'bluetooth earbuds', 'gaming accessory',
        'usb c hub', 'ai gadget', 'robot vacuum', 'smartwatch', 'dashcam', 'portable projector'
      ],
      fashion: [
        'sustainable fashion', 'athleisure', 'capsule wardrobe', 'oversized hoodie',
        'minimal jewelry', 'vegan leather', 'streetwear', 'retro sneakers', 'linen dress', 'cargo pants'
      ],
      beauty: [
        'skin barrier', 'retinol serum', 'spf 50', 'glass skin', 'hair oil',
        'scalp care', 'clean beauty', 'lip oil', 'niacinamide', 'vitamin c serum'
      ],
      home: [
        'air purifier', 'cordless vacuum', 'smart lighting', 'organizer box', 'desk setup',
        'standing desk', 'ergo chair', 'coffee grinder', 'scented candles', 'water filter'
      ],
      fitness: [
        'resistance bands', 'yoga mat', 'massage gun', 'pre workout', 'running shoes',
        'home gym', 'protein shaker', 'smart scale', 'foam roller', 'fitness tracker'
      ],
      outdoor: [
        'camping gear', 'hiking backpack', 'portable solar', 'water filter straw', 'bike lights',
        'trail running', 'trekking poles', 'bug repellent', 'sleeping pad', 'camp stove'
      ],
      pets: [
        'cat fountain', 'automatic feeder', 'pet camera', 'dog harness', 'lick mat',
        'smart litter box', 'cat scratching post', 'dog chew toy', 'gps tracker', 'pet shampoo'
      ],
      toys: [
        'stem toys', 'magnetic tiles', 'montessori', 'fidget toy', 'wooden puzzles',
        'coding for kids', 'robot kit', 'outdoor play', 'sensory toys', 'balance bike'
      ],
      digital: [
        'creator tools', 'podcast mic', 'stream deck', 'video lights', 'green screen',
        'productivity app', 'note taking', 'time tracking', 'password manager', 'ai assistant'
      ],
      ai: [
        'ai prompts', 'image generation', 'ai copywriting', 'rpa automation', 'vector database',
        'langchain', 'llm fine tuning', 'agents', 'prompt engineering', 'ai seo'
      ]
    };
     ...
     */
    fastify.get<{
      Querystring: { category?: string; specialization?: string; limit?: string; sources?: string };
    }>('/trending-keywords', {
      schema: {
        ...
        querystring: {
          type: 'object',
          properties: {
            category: { type: 'string', description: 'Category seed set (e.g., electronics, fashion)' },
            specialization: { type: 'string', description: 'Optional specialization to bias seeds' },
            limit: { type: 'string', description: 'Max number of keywords to return (default 10)' },
      ...
      try {
        ...
        const limitNum = Math.max(1, Math.min(50, Number(_request.query.limit || 10)));
        ...
        if (!seeds.length) {
          ...
          seeds = [
            'best sellers', 'new release', 'discount', 'gift ideas', 'eco friendly',
            'smart home', 'wireless', 'portable', 'minimalist', 'premium'
          ];
        }
        ...
        logger.info({ category, specialization, seeds: seeds.length }, 'Trending keywords analysis');
        ...
        const ranked = sources && sources.length
          ? await Promise.all(results.map(async r => {
              const aggr = await trendAggregator.aggregateTrends(r.keyword, sources);
              return { keyword: r.keyword, score: aggr.overallScore, confidence: aggr.confidence };
            }))
          : results.map(r => ({ keyword: r.keyword, score: r.overallScore, confidence: r.confidence }));
