...
export const trendAggregatorRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    ...
    {
      schema: {
        description: 'Get trend analysis history',
        tags: ['Trends'],
        response: {
          200: {
            type: 'object',
    ...
    async (_request, _reply) => {
      try {
        ...
        const history = [
          {
            ...
            trendsFound: Math.floor(Math.random() * 20) + 10,
            duration: `${Math.floor(Math.random() * 5) + 1}.${Math.floor(Math.random() * 10)}s`,
            status: 'completed',
          },
          {
        ...
        _reply.code(500);
        return { error: 'Failed to get analysis history' };
      }
    }
  );
