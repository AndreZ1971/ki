...
interface ImageClassification {
  type: 'product' | 'portrait' | 'landscape' | 'screenshot' | 'icon' | 'graphic' | 'other';
  subType: string;
  confidence: number;
  useCase: string[];
}
...
function calculateQualityScore(metadata: any): QualityScore {
  ...
  const contrastScore = metadata.density ? Math.min(100, (metadata.density / 72) * 100) : 70;

  // Sharpness Score (estimate from format/size)
  const sharpnessScore = metadata.format === 'png' ? 90 : metadata.format === 'webp' ? 85 : 75;

  // Color Profile Score
  const colorProfileScore = metadata.hasAlpha ? 100 : 80;

  // Overall Quality
  const overallQuality = Math.round((resolutionScore + contrastScore + sharpnessScore + colorProfileScore) / 4);
              ...
              text: `Du bist ein Bildanalyse- und SEO-Experte. Analysiere das Bild und gib JSON zurück:
{
  ...
  "textContent": "Wenn Text im Bild vorhanden"
    ...
    return jsonMatch ? JSON.parse(jsonMatch[0]) : { tags: [], description: '', alt: '', filename: '' };
  } catch (_error) {
    return { tags: [], description: '', alt: '', filename: '' };
  }
}
...
function enhanceTagsWithConfidence(tags: string[]): TagResult[] {
  const tagCategories = {
    object: ['product', 'person', 'animal', 'object', 'building', 'car', 'food', 'plant'],
    style: ['minimalist', 'professional', 'playful', 'bold', 'elegant', 'modern', 'vintage'],
    emotion: ['happy', 'serious', 'energetic', 'calm', 'inspiring', 'sad', 'funny'],
    action: ['running', 'jumping', 'working', 'playing', 'sitting', 'standing', 'moving'],
    context: ['outdoor', 'indoor', 'portrait', 'landscape', 'closeup', 'wide-shot']
  };

  return tags.map((tag, index) => {
    let category: 'object' | 'style' | 'emotion' | 'action' | 'context' = 'object';
...
function generateSEORecommendations(vision: any, quality: QualityScore, _metadata: any): SEORecommendations {
  const recommendations: string[] = [];
  let accessibilityScore = 80;

  // Alt-Text Quality
  if (!vision.alt || vision.alt.length < 20) {
    recommendations.push('Alt-Text sollte aussagekräftiger sein (mindestens 20 Zeichen)');
  ...
  // Filename
  if (!vision.filename) {
    recommendations.push('Verwende aussagekräftigen Dateinamen statt generischer Namen');
    accessibilityScore -= 5;
  }

  // Resolution
  if (quality.resolution === 'low') {
    recommendations.push(`Auflösung zu niedrig (${quality.width}x${quality.height}). Ideal: min. 800x800px`);
...
function generateOptimizationSuggestions(metadata: any, quality: QualityScore, _vision: any): OptimizationSuggestion[] {
  ...
  if (metadata.format === 'jpg') {
    suggestions.push({
      ...
      implementation: 'sharp().jpeg({ quality: 82, progressive: true }).toFile(...)'
    });
  }
...
function classifyImage(vision: any, metadata: any): ImageClassification {
  ...
  let type: 'product' | 'portrait' | 'landscape' | 'screenshot' | 'icon' | 'graphic' | 'other' = 'other';
  let confidence = 0.5;
  ...
  return {
    ...
    useCase: type === 'product' ? ['ecommerce', 'catalog', 'social-media'] : ['blog', 'article', 'social-media']
  };
}
