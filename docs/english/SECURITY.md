...
### License-Critical Constraints

The binding legal terms are defined in [LICENSE](../../LICENSE). For security and licensing consistency:

- Specialization signature verification must not be removed, disabled, bypassed, or circumvented.
- The embedded verifier public key must not be replaced by a different key.
