...
```
SZENARIO: A.R.I. 1.0.0 ist verfügbar (bessere Tools, Bugfixes)
...
2. Automattic rollt aus (Kubernetes Rolling Update)
   ...
   ↓
3. Neuer Container (1.0.0) startet mit:
   - Mode: update
   - Alte connection.json (1.0.0 Container) als Input
   ↓
...
# NEU (1.0.0) - nur 1 Zeile geändert!
services:
  app:
    image: ari:1.0.0      # ← NUR DIESE ZEILE
    container_name: woo-app-prod
    # ...
```
...
→ Starte Deployment mit A.R.I. Image 1.0.0
```
