...
```
SZENARIO: A.R.I. 7.5.0 ist verfügbar (bessere Tools, Bugfixes)
...
2. Automattic rollt aus (Kubernetes Rolling Update)
   ...
   ↓
3. Neuer Container (7.5.0) startet mit:
   - Mode: update
   - Alte connection.json (7.5.0 Container) als Input
   ↓
...
# NEU (7.5.0) - nur 1 Zeile geändert!
services:
  app:
    image: ari:7.5.0      # ← NUR DIESE ZEILE
    container_name: woo-app-prod
    # ...
```
...
→ Starte Deployment mit A.R.I. Image 7.5.0
```
