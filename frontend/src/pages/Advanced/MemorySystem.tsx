...
import { formatDateTime, formatNumber } from "../../lib/i18n-utils";
import "./page.css";
...
const MemorySystem: React.FC = () => {
  ...
  const ttlSeconds = useMemo(
    () => Math.max(60, Math.min(86400, Number(ttl) || 0)),
    [ttl]
  );
  const maxEntriesNum = useMemo(
    () => Math.max(100, Math.min(10000, Number(maxEntries) || 0)),
    [maxEntries]
  );
  ...
  const handleOptimize = async () => {
    ...
    setError(null);
    try {
      const res = await memoryApi.optimize({
        maxEntries: maxEntriesNum,
        ttlSeconds,
      });
      if (!res.success || !res.data)
        throw new Error(res.error || "Optimierung fehlgeschlagen");
      setMemoryStats(res.data);
      showToast("Memory optimiert", "success");
      await loadMessages();
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : "Optimierungsfehler";
  ...
  const handleClearCache = async () => {
    ...
    try {
      ...
      if (!res.success)
        throw new Error(res.error || "Konnte Memory nicht leeren");
      setMemoryStats({
        totalMessages: 0,
        memorySize: 0,
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Fehler beim Leeren";
      setError(errorMessage);
      showToast(errorMessage, "error");
    } finally {
      setLoading(false);
  ...
  const lastClearedDisplay = memoryStats?.lastCleared
    ? formatDateTime(memoryStats.lastCleared)
    : "–";
          ...
          {memoryStats ? (
            ...
            >
              ...
              >
                ...
                >
                  {formatNumber(memoryStats.totalMessages)}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                >
                  {formatBytes(memoryStats.memorySize)}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                >
                  Limit: {maxEntriesNum} • TTL: {ttlSeconds}s
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                >
                  Zuletzt gespeicherte Nachrichten
                </div>
                {loadingMessages ? (
                  <div style={{ color: "rgba(255,255,255,0.7)" }}>Lade…</div>
                ) : messages.length ? (
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    {messages.map((msg, idx) => (
                      ...
                      >
                        ...
                        >
                          {msg.role} • {formatDateTime(msg.timestamp)}
                        </div>
