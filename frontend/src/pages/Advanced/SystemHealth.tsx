...
const SystemHealth: React.FC = () => {
  ...
  const handleCheckHealth = async () => {
    ...
    try {
      ...
      const overThreshold =
        metrics.cpu.usage > threshold ||
        metrics.memory.usagePercent > threshold ||
        metrics.disk.usagePercent > threshold;
      if (monitoringEnabled && overThreshold) {
        showToast(`Alert: Ressourcen über ${threshold}%`, "error");
      }

      showToast(
        `System-Status: ${metrics.status === "healthy" ? "Gesund ✅" : metrics.status === "warning" ? "Warnung ⚠️" : "Kritisch 🚨"}`,
        metrics.status === "healthy" ? "success" : "error"
      );
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : "Health-Check Fehler";
  ...
  return (
    <div className="page-container">
        ...
        >
          ...
          <div style={{ marginTop: "20px" }}>
            <LoadingButton
              ...
              loading={loading}
              loadingText="Prüfe System..."
            >
              ⚙️ Health-Check Starten
            </LoadingButton>
          </div>
        </motion.div>
              ...
              >
                <div style={{ marginBottom: "15px" }}>
                  ...
                  >
                    <div
                      style={{
                        width: `${healthStatus.cpu}%`,
                        height: "100%",
                        background: getStatusColor(healthStatus.cpu),
                ...
                <div style={{ marginBottom: "15px" }}>
                  ...
                  >
                    <div
                      style={{
                        width: `${healthStatus.memory}%`,
                        height: "100%",
                        background: getStatusColor(healthStatus.memory),
                        transition: "width 0.5s",
                      }}
                    />
                  </div>
                ...
                <div style={{ marginBottom: "15px" }}>
                  ...
                  >
                    <div
                      style={{
                        width: `${healthStatus.disk}%`,
                        height: "100%",
                        background: getStatusColor(healthStatus.disk),
                        transition: "width 0.5s",
                      }}
                    />
                  </div>
      ...
      <FloatingChatbot
        ...
        greetings={[
          "Hi, ich bin Ari. Frag mich nach dem System-Status, Monitoring oder Fehlern.",
        ]}
      />
    </div>
  );
