...
import { formatDateTime, formatTime } from "../../lib/i18n-utils";
import "./page.css";
...
const WooCommerceSync: React.FC = () => {
  ...
  const syncTypes: {
  ...
  }[] = [
    {
      ...
      label: "Vollständig",
      icon: "🔄",
      description: "Alle Daten",
    },
    {
      value: "products",
      label: "Nur Produkte",
      icon: "🛍️",
      description: "Produkt-Sync",
    },
    {
      value: "orders",
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      setSyncStats({
        ...
        lastSync: formatDateTime(stats.lastSync),
      ...
      if (autoSync) {
        ...
        setNextRun(formatTime(next));
      }
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : "Synchronisationsfehler";
        ...
        >
            ...
            >
              {syncTypes.map((type) => (
                <motion.div
                  ...
                  whileTap={{ scale: 0.97 }}
                  onClick={() => setSyncType(type.value)}
                  style={{
                    padding: "14px",
                    background:
                      syncType === type.value
          ...
          {autoSync && (
            <div className="form-group">
              ...
              >
                Min. 5 Minuten, aktuell: {intervalMinutes} min
              </div>
            </div>
          )}
