  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const healthEmoji = response.data.systemHealth === 'healthy' ? '✅' : 
                           response.data.systemHealth === 'degraded' ? '⚠️' : '🚨';
        
        showToast(
          `${healthEmoji} ${issueCount} Issue(s) erkannt - System: ${response.data.systemHealth}`, 
          issueCount === 0 ? 'success' : response.data.systemHealth === 'critical' ? 'error' : 'info'
        );
      }
    } catch (err) {
  ...
  return (
    <div className="page-container">
      ...
      <motion.div className="page-header" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <h1>🤖 Payment Issue Detector (KI-Powered)</h1>
        <p>GPT-4o-mini gestützte Echtzeit-Analyse des Payment-Systems</p>
      </motion.div>
      ...
      {detectionResult && (
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
            <div>
              <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', marginBottom: '5px' }}>System Health</div>
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: getHealthColor(detectionResult.systemHealth), textTransform: 'uppercase' }}>
                {detectionResult.systemHealth === 'healthy' && '✅ HEALTHY'}
                {detectionResult.systemHealth === 'degraded' && '⚠️ DEGRADED'}
                {detectionResult.systemHealth === 'critical' && '🚨 CRITICAL'}
              </div>
            </div>
            <div>
              <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', marginBottom: '5px' }}>KI Confidence</div>
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#007aff' }}>
                {Math.round(detectionResult.overallConfidence * 100)}%
              </div>
            </div>
            <div>
              <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', marginBottom: '5px' }}>Analyzed Events</div>
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: 'white' }}>
                {detectionResult.scanMetadata.scannedEvents}
              </div>
            </div>
            <div>
              <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.6)', marginBottom: '5px' }}>Failure Rate</div>
              <div style={{ fontSize: '24px', fontWeight: 'bold', color: detectionResult.scanMetadata.currentFailureRate > 0.1 ? '#ff3b30' : '#34c759' }}>
                {(detectionResult.scanMetadata.currentFailureRate * 100).toFixed(1)}%
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div className="form-container" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="form-group">
            ...
            <select value={scanDepth} onChange={(e) => setScanDepth(e.target.value as 'quick' | 'standard' | 'deep')} className="form-input">
              <option value="quick">🚀 Schnell (Oberflächlich)</option>
              <option value="standard">⚡ Standard (Empfohlen)</option>
              <option value="deep">🔬 Tiefgehend (Gründlich)</option>
            </select>
          </div>
          ...
          <div style={{ marginTop: '20px' }}>
            <LoadingButton onClick={handleScan} loading={loading} loadingText="🔍 KI scannt...">
              🤖 KI-Scan starten
            </LoadingButton>
          </div>
        </motion.div>
        ...
        <motion.div className="result-container" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {detectedIssues.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {detectedIssues.map((issue: PaymentIssue, idx: number) => (
                ...
                >
                  ...
                  {/* Description */}
                  <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.8)', marginBottom: '12px', lineHeight: '1.5' }}>
                    {issue.description}
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <div style={{ fontSize: '11px', fontWeight: '600', color: '#007aff' }}>
            ...
            <div style={{ 
              ...
              <div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '8px' }}>Keine Issues erkannt</div>
      ...
      {detectionResult && detectionResult.recommendedActions.length > 0 && (
        ...
        >
          ...
          <ul style={{ margin: 0, paddingLeft: '20px', color: 'rgba(255,255,255,0.8)', lineHeight: '1.8' }}>
            {detectionResult.recommendedActions.map((action: string, idx: number) => (
              <li key={idx} style={{ fontSize: '14px' }}>{action}</li>
            ))}
          </ul>
        </motion.div>
      )}
