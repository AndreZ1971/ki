...
const PaymentFast: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    // Check fraud risk
    if (fraudAnalysis && fraudAnalysis.riskLevel === 'high' && fraudAnalysis.riskScore > 80) {
      const confirm = window.confirm(`⚠️ Hohes Betrugsrisiko (${fraudAnalysis.riskScore}%)!\n\n${fraudAnalysis.recommendation}\n\nTrotzdem fortfahren?`);
      if (!confirm) return;
    }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '20px', marginTop: '20px' }}>
        <motion.div className="form-container" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div style={{ marginBottom: '15px' }}>
            ...
            >
              {loadingSuggestions ? '⏳ Lade...' : '✨ Smarte Betragsempfehlungen'}
            </button>
          </div>
          ...
          {fraudAnalysis && (
            <div style={{ 
              background: fraudAnalysis.riskLevel === 'high' ? 'rgba(255, 59, 48, 0.1)' : 
                          fraudAnalysis.riskLevel === 'medium' ? 'rgba(255, 159, 10, 0.1)' : 
                          'rgba(52, 199, 89, 0.1)',
              border: `1px solid ${fraudAnalysis.riskLevel === 'high' ? 'rgba(255, 59, 48, 0.5)' : 
                                   fraudAnalysis.riskLevel === 'medium' ? 'rgba(255, 159, 10, 0.5)' : 
                                   'rgba(52, 199, 89, 0.5)'}`,
              borderRadius: '12px',
              padding: '15px',
              marginBottom: '15px'
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span style={{ fontSize: '13px', fontWeight: '600', color: 'white' }}>
                  {fraudAnalysis.riskLevel === 'high' ? '⚠️ Hohes Risiko' : 
                   fraudAnalysis.riskLevel === 'medium' ? '⚡ Mittleres Risiko' : 
                   '✅ Niedriges Risiko'}
                </span>
              ...
              {fraudAnalysis.flags.length > 0 && (
                <div style={{ marginTop: '10px', fontSize: '11px', color: 'rgba(255,255,255,0.9)' }}>
                  {fraudAnalysis.flags.map((flag, idx) => (
                    <div key={idx} style={{ marginBottom: '4px' }}>• {flag}</div>
                  ))}
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              {fraudAnalysis && (
                <div style={{ 
                  ...
                  <h4 style={{ color: 'white', fontSize: '14px', marginBottom: '15px' }}>🛡️ Sicherheitsanalyse</h4>
                  
                  <div style={{ marginBottom: '15px' }}>
                    <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.7)', marginBottom: '8px' }}>Risiko-Score</div>
                    <div style={{ 
                      ...
                      <div style={{ 
                        ...
                        height: '100%',
                        background: fraudAnalysis.riskLevel === 'high' ? 
                  ...
                  {fraudAnalysis.flags.length > 0 && (
                    <div style={{ marginBottom: '15px' }}>
                      <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.7)', marginBottom: '8px' }}>Erkannte Flags</div>
                      {fraudAnalysis.flags.map((flag, idx) => (
                        <div key={idx} style={{ 
                          fontSize: '12px', 
                          color: 'white',
                  ...
                  <div>
                    <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.7)', marginBottom: '8px' }}>Empfehlung</div>
                    <div style={{ 
                      fontSize: '12px', 
                      color: 'white',
                      background: 'rgba(102, 126, 234, 0.1)',
                  ...
                  {fraudAnalysis.confidence && (
                    <div style={{ marginTop: '15px', fontSize: '11px', color: 'rgba(255,255,255,0.5)', textAlign: 'center' }}>
