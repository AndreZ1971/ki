...
async function main() {
  ...
  const shortDesc =
    (args.short as string) ??
    `Kostenloses Wallpaper-Bundle (Dark & Light) als ZIP-Download.`;
  const longDesc =
    (args.long as string) ??
    `<p>Minimalistische Wallpaper in 4K/1440p/1080p + Mobile-Formate. ZIP enthalten.</p>`;
